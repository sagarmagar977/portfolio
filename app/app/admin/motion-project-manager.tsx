...
import { deleteMotionProjectAction, reorderMotionProjectsAction, upsertMotionProjectAction } from "./actions";
...
type MotionProjectManagerProps = {
  profileId: string;
  motionProjects: MotionProjectItem[];
};
...
export function MotionProjectManager({ profileId, motionProjects }: MotionProjectManagerProps) {
  const orderedProjectsFromProps = useMemo(
    () => [...motionProjects].sort((a, b) => a.sortOrder - b.sortOrder),
    [motionProjects],
  );
          ...
          <div className="admin-modal-shell">
            <div className="admin-modal-card">
              <div className="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3 mb-3">
                <div>
                  <h3 className="h4 mb-1">{draft.id ? "Edit Motion Piece" : "Add Motion Piece"}</h3>
                  <p className="admin-muted mb-0">Support image, GIF, and video previews for 3D or animated work.</p>
                </div>
                <button type="button" className="btn btn-outline-light" onClick={closeModal}>Close</button>
              </div>

              <form action={upsertMotionProjectAction} className="row g-3">
                ...
                <div className="col-md-6">
                  <label className="form-label">Tool / Software</label>
                  <input className="form-control" name="toolName" value={draft.toolName} onChange={(event) => setDraft((current) => current ? { ...current, toolName: event.target.value } : current)} />
                </div>
                ...
                <div className="col-md-4">
                  <label className="form-label">Upload Preview Image</label>
                  <input className="form-control" type="file" name="previewImageFile" accept="image/*" />
                </div>

                <div className="col-md-4">
                  <label className="form-label">Upload GIF</label>
                  <input className="form-control" type="file" name="previewGifFile" accept="image/gif" />
                </div>

                <div className="col-md-4">
                  <label className="form-label">Upload Video</label>
                  <input className="form-control" type="file" name="previewVideoFile" accept="video/*" />
                </div>
          ...
          >
            <div className="admin-entity-summary">
              <div className="d-flex align-items-start gap-3 flex-wrap">
                {project.previewImageUrl || project.previewGifUrl ? (
                  <img src={project.previewGifUrl ?? project.previewImageUrl ?? ""} alt={project.title} className="admin-project-thumb rounded-3" />
                ) : (
                  <div className="admin-project-thumb admin-project-thumb-empty rounded-3">No preview</div>
                )}
                <div>
                  <div className="d-flex align-items-center gap-2 flex-wrap mb-1">
                    <h3 className="h5 mb-0">{project.title}</h3>
                    {project.toolName ? <span className="admin-service-badge">{project.toolName}</span> : null}
                  </div>
            ...
            <div className="admin-entity-actions">
              ...
              <form action={deleteMotionProjectAction}>
